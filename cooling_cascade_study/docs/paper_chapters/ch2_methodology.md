...
|---|---|---|---|---|
| B 市政配水 | D-town 管网 | 延时水力模拟（EPS）＋压力驱动需水（PDD） | 小时级（分区水箱缓冲耗尽） | 节点压头 $H_{\mathrm{muni}}$ |
...
式 (2.1) 中的 $(t_{\mathrm{fault}},\tau_r)$ 与各取水节点的 $t_{\mathrm{fail}}$ 并非假设，而是由真实城市管网水力仿真生成（`saet_distribution.py`、`full_coupling_boundary.py`）：市政侧采用 D-town 基准管网 [4,5]，以 EPANET 2 延时模拟（EPS）与压力驱动需水（PDD）模式求解 [7,8]，步长 15 min、观测窗 72 h，节点需水按管网自带的 5 条日变化模式（DMA1_pat–DMA5_pat，各 168 个逐时乘子）分配。PDD 采用 Wagner 型压力-流量关系 [15]：
...
| 主动控制 LP | `run_p6.py` | $\Delta t=5$ min，时域 300 min（v2：覆盖 6 台最大 SAET 185.5 min 的 α=1.5 软着陆窗口；级联/DISP 另按簇窗自动延长） | HiGHS；`ProactiveLP` 类默认 40 min/2 min/0.02 min⁻¹ |
