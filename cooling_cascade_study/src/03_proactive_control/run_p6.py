...
v2 (2026-09-15 重构): 受影响机组集合改为耦合层 coupling_map.json 指定的
**6 台** (bus 89/80/10/66/65/26, ΣPg=2631 MW); LP 时域由 200 min 扩至
**300 min** —— 因 6 台 SAET 最大 185.5 min (bus26), DP 控制时间 1.5×SAET
≈278 min, 200 min 时域会截断软着陆。
"""
import os, sys, json
...
def run(affected=None, horizon_min=300, dt_min=5.0,
        ramp=0.01, alpha=1.5, enforce_dc=True):
    if affected is None:
        affected = wi.coupled_buses()
