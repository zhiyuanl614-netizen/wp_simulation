...
三组仿真 (均无补水反馈, EPANET 2.2, PDD required=20/min=0, 15 min):
  ...
  saet     : R1 自 t=0 阶跃停供 72h -> 重算 t_fail, 与 coupling_map 分层对比;
             v2 起 plants[bus] 增存 pressure_saet_m + t_fail_saet_h (B-ST 合成
             阶跃口径, 供 Fig5 绘制——与 Fig6/分层定义同口径; boundary 既有
             字段保持逐位不变, 下游 full_order_cascade 零影响)
...
if __name__ == "__main__":
    ...
    for r in rows:
        ...
        # v2: 存储 saet (B-ST) 口径轨迹, 供 Fig5 绘制
    ...
    print("  saet 前锋(B-ST): 失效 %d/54, 范围 %.2f-%.2f h, t=0 即时失效 %d, never %d"
          % (len(tv), min(tv), max(tv),
             sum(1 for t in tv if t == 0.0), 54 - len(tv)))
