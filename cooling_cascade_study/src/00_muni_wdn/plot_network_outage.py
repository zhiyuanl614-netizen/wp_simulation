...
Fig.6  Spatio-temporal pressure collapse under unique-source outage.
  ...
  bottom: (c1)-(c4) 2x2 spatial snapshots (nodes coloured by pressure, RdYlBu 0-80 m
        -- same scale as Fig.5; grey pipes, black square = source R, green triangles = tanks)
...
def main():
    ...
    for k, h in enumerate(o["snap_hours"]):
        ...
        px = [v[0] for v in o.get("pumps", {}).values()]
        py = [v[1] for v in o.get("pumps", {}).values()]
        ax.scatter(px, py, s=90, facecolor="white", edgecolor="#333333",
                   linewidths=1.0, zorder=5)
        ax.scatter(px, py, s=34, marker="^", facecolor="#5b6b7a",
