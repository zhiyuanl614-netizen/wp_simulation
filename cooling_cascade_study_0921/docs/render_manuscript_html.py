...
EXPECT = {"img": 12, "table": 7, "figure": 19}  # 12 图 + 表 1-7 刊内编号（#31 规范化：Table 1/3/5/7 与 Fig 5/6 移入补充材料 S1/S1.4/S2.4/S3.1/S3.2）; figure=图12+表包装7
...
def main():
    ...
    def wrap_img(m):
        n_fig_img[0] += 1
        return (f'<figure><img src="{m.group(1)}" alt="{m.group(2)}{m.group(3)}">'
                f"<figcaption>{m.group(4)}</figcaption></figure>")

    body = re.sub(
        r'<p><img src="([^"]+)" alt="(图|Fig\.)(\d+)"\s*/></p>\s*<p>(<strong>(?:图 \3　|Fig\. \3 )[\s\S]*?)</p>',
        wrap_img, body)
    ...
    body = re.sub(
        r"<p>(<strong>(?:表|Table) (\d+)[\s\S]*?</strong>.*?)</p>\s*(<table>[\s\S]*?</table>)",
        wrap_tab, body)
    ...
    # ---------- 7) 参考文献悬挂缩进 ----------
    m = re.search(r'(<h1 id="sec-\d+">References</h1>)([\s\S]*?)(<h1 id=|$)', body)
    assert m, "未找到参考文献小节"
...
<title>Early-Warning-Driven Proactive Resilience Optimization of Urban Water–Power Cyber-Physical Systems (full manuscript)</title>
<style>{css}</style>
</head>
<body>
{body}
<footer>Full manuscript · self-contained HTML rendered by docs/render_manuscript_html.py (images and MathJax embedded; offline-viewable and printable) · {TODAY}</footer>
{mathjax_tags}
</body>
</html>
"""
    ...
    if n_fig_tab[0] != EXPECT["table"]:
        problems.append(f"表 figure 包装 {n_fig_tab[0]} ≠ {EXPECT['table']}")
    if page.count("<figure") != EXPECT["figure"]:
        problems.append(f"figure 总数 {page.count('<figure')} ≠ {EXPECT['figure']}")
    if problems:
        print("!! 渲染校验未通过：")
        for p in problems:
            print("   -", p)
