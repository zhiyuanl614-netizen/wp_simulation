...
停供: 唯一水源 R1 自 t=0 阶跃停供 (总水头置近零, 分区水箱重力放水续供); PDD。
失压阈值 = 28 m (厂侧补水充填压头判据); 健康阈值 = 32 m (基线 24 h 最小压头)。

输出:
  results/saet_distribution.json  全网逐节点失压时刻 + 分布统计 + P10/P50/P90 代表节点
  figures/Fig12_depressurization_time_distribution.png (由 plot_saet_distribution.py 绘制:
    ① 失压时刻直方图 ② 累计失压占比(CDF) ③ 空间分布着色)
...
def analyze(duration_h=72.0, step_min=15, save=True):
    ...
    out = dict(
        ...
        pumps={pn: [round(0.5 * (wn.get_node(wn.get_link(pn).start_node_name).coordinates[0] +
                                 wn.get_node(wn.get_link(pn).end_node_name).coordinates[0]), 2),
                    round(0.5 * (wn.get_node(wn.get_link(pn).start_node_name).coordinates[1] +
                                 wn.get_node(wn.get_link(pn).end_node_name).coordinates[1]), 2)]
               for pn in wn.pump_name_list},
        reservoir_coord=[round(float(wn.get_node(wn.reservoir_name_list[0]).coordinates[0]), 2),
                         round(float(wn.get_node(wn.reservoir_name_list[0]).coordinates[1]), 2)],
