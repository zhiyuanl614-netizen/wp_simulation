"""
指定耦合边界 + 一致性验证 + 补水可供性评估 (语义 ①: 补水不反馈进城市水力)
=========================================================================
v2 (2026-09-15 重构): 随 coupling_map v2 由"全 54 对 + Q1–Q5 分层"改为
【研究者指定 6 对】; 取消全部分层汇总, 一致性验证改为"逐节点 t_fail 复算一致率"。

背景 (docs/coupling_map.md §全耦合验证): 把额定补水反馈进 D-town 基线不可行
(v1 全耦合 54 路 299.7 L/s ≫ 城市水源出力 ~246 L/s, 耦合节点 min 压头 −12 m;
即使昼间模式+总量预算可行, 任何非零反馈都会耗尽水箱缓冲使 never 层 (~24 h) 失压,
与已发布分布矛盾)。v2 六路额定补水合计仅 95.7 L/s, 但仍沿用同一语义以保持
与 saet_distribution / coupling_map 的口径一致、并使下游结果可比:
城市水力保持无补水口径, 额定补水作为【被评估的需求】事后用 PDD 计算可供比例。

本模块输出 (results/muni/):
  full_coupling_boundary.json   6 母线耦合边界 (boundary 口径压头轨迹) + 可供性
  full_coupling_consistency.csv 逐母线: coupling_map 登记 t_fail vs 本模块重算 (应 100%)

三组仿真 (均无补水反馈, EPANET 2.2, PDD required=20/min=0, 15 min):
  baseline : 无故障 24h            -> 健康/可行性 (耦合节点 min pressure)
  saet     : R1 自 t=0 阶跃停供 72h -> 重算 t_fail, 与 coupling_map 对比;
             plants[bus] 增存 pressure_saet_m + t_fail_saet_h (B-ST 合成阶跃口径,
             供 Fig5 绘制——与 Fig6 同口径)
  boundary : R1 自 t=6h 起 3h 降压  -> 6 条压头轨迹 + t_fail_after_fault_s (下游输入)
可供性评估: 对 boundary 轨迹, 按 EPANET PDD 供水份额 f(p)=sqrt(clip(p/20,0,1))
...
if __name__ == "__main__":
    ...
    for r in rows:
        ...
        cons.append(dict(rank=r["rank"], bus=r["bus"], junction=r["junction"],
                         demand_Lps=r["demand_Lps"],
                         t_fail_map_h=r["t_fail_h"],
                         t_fail_new_h=(round(t_new, 3) if t_new is not None else None),
                         agree=(r["t_fail_h"] == (round(t_new, 3) if t_new is not None
                                                  else None))))
        saet_pr[str(r["bus"])] = dict(
            t_fail_saet_h=(round(t_new, 3) if t_new is not None else None),
            pressure_saet_m=[round(float(x), 2) for x in h])
    agree = sum(1 for c in cons if c["agree"])
    print("== [2/3] saet 口径一致性 (应=100%%) ==")
    print("  t_fail 完全一致 %d/%d (%.1f%%)" % (agree, N, 100.0 * agree / N))
    tv = sorted(c["t_fail_new_h"] for c in cons if c["t_fail_new_h"] is not None)
    print("  saet 前锋(B-ST): 失效 %d/%d, 范围 %.2f-%.2f h, t=0 即时失效 %d, never %d"
          % (len(tv), N, min(tv), max(tv),
             sum(1 for t in tv if t == 0.0), N - len(tv)))
    ...
    for r in rows:
        ...
        plants[str(r["bus"])] = dict(
            rank=r["rank"], node=j, makeup_Lps=r["makeup_Lps"],
            demand_Lps=r["demand_Lps"], head0_m=round(head0, 2),
            ...
            **saet_pr[str(r["bus"])])
        avail_rows.append(dict(rank=r["rank"], bus=r["bus"], junction=j,
                               avail_frac_at_fault=round(float(f[i6]), 3),
                               t_fail_boundary_h=(round(t_fail, 2)
                                                  if t_fail is not None else None),
    ...
    for a in avail_rows:
        print("  #%d bus%-4d %-6s  72h需求 %8.0f m3 / 可供 %8.0f m3 (%5.1f%%)  "
              "t_fail(B-RT)=%s h  24h内断供=%s"
              % (a["rank"], a["bus"], a["junction"], a["requested_72h_m3"],
                 a["delivered_72h_m3"], 100 * a["delivery_ratio"],
                 "-" if a["t_fail_boundary_h"] is None else f"{a['t_fail_boundary_h']:.2f}",
                 "是" if (a["t_to_zero_h"] or 99) <= 24 else "否"))
    print("  合计: 需求 %.0f m3, 可供 %.0f m3 (%.1f%%)"
          % (tot_req, tot_del, 100 * tot_del / tot_req))

    out = dict(
        source=cm["source"], network="DTOWN.inp", version="v2",
        ...
        makeup_rated_total_Lps=round(mk_total, 1), n_coupled=N,
        ...
        consistency=dict(agree_exact=agree, n=len(cons), rows=cons),
        ...
        makeup_availability=dict(pdd="f=sqrt(clip(p/20,0,1))",
                                 ...
                                 total_delivery_ratio=round(tot_del / tot_req, 3),
                                 rows=avail_rows),
        plants=plants,
    )
