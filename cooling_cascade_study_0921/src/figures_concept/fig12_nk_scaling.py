...
数据核实 v2 (2026-09-15, 耦合层重构): 总体改为耦合层指定的 6 台凝汽式机组
(ΣPg=2631 MW), k=1..6 全枚举 × (worst + 3 随机) = 24 子集, horizon 300 min;
SP 零缺额 24/24 (SUET 预警 88.6-185.5 min 足以使任意 ≤6 台组合软着陆; v4 统一语义后 92.4-192.9, 24/24 不变);
PA 能量缺额在 k>=2 后饱和于 12-13 MWh (峰值 ~148-155 MW 受剩余机组爬坡
限制而非失出力规模), 与 v1 (19 台总体, k=1..12) 的线性增长定性不同;
k=3 worst = [89, 80, 10] (6 台总体按 Pg 降序 top-3)。
"""
import os, sys, json
import numpy as np
...
axA.plot(kw, sp, "s-", color=COLORS["SP"], label="SP (proactive, SUET)")
