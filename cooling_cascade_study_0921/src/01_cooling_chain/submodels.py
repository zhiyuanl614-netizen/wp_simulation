...
P1 故障机理链子模型（W + A）
============================
W: 水源侧水力学   -> 循环水流量 m_cw, 进水温 T_cw_in, 泵状态
A: 凝汽器-低压缸  -> 背压 p_b, 机械功率降额 k_p (-> 机组少发功率)
故障源: 市政配水节点压头 < 最小供水阈值(28m) => 补水中断

说明: 本模块为 ICS 与 P6 共用的物理层子模型。影响以少发功率/损失电量衡量,
不含系统频率计算(与直流潮流口径一致)。
"""
import numpy as np
...
# ---------------- 子模型 W: 水力 ----------------
def make_flow(t, H_tank, p, t_fault, ramp, cap_m3s=None):
    """市政->补水箱 补水流量 m^3/s (市政压力直供 + 液位控制阀)。
    失效源: 配水节点压头 < 最小供水阈值 H_muni_min(28m) => 无法为补水箱补水 => 补水归零。
    正常时: 速率 = Cv·开度·√(有效压头); 液位越接近设定值, 阀开度越小(闭环)。
    cap_m3s: 补水封顶 m^3/s (None=不封顶)。评审M2 封顶敏感性实验用;
             默认 None 与原实现逐字节等价(既有调用方均不传此参)。"""
    head = muni_head(t, p, t_fault, ramp)
    if head < p.H_muni_min:                          # 失效判据: 低于最小供水阈值即断供
        return 0.0
    ...
    q = p.Cv_make * opening * np.sqrt(driving)
    if cap_m3s is not None:
        q = min(q, cap_m3s)
    return q
...
def water_derivs(t, H_tank, H_pool, pump_tripped, p, t_fault, ramp, Q_cond_kW=None,
    ...
    make_cap_m3s: 市政补水封顶(m^3/s, None=不封顶, 评审M2 实验); 传递给 make_flow。"""
    if Q_cond_kW is None:
        Q_cond_kW = p.Q_cond0
    qmake = make_flow(t, H_tank, p, t_fault, ramp, cap_m3s=make_cap_m3s)   # 市政->补水箱
