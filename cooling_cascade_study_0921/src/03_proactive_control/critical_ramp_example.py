...
def asw_of(p, Ht, Hp):
    """冷却水可用储备量 CWR (m^3)：补水箱+集水池 相对跳泵最低水位。"""
...
def main():
    ...
    print(f"\n临界: 1/r* = {crit['ramp_time_s']:.0f} s "
          ...
          f"T_max = {crit['T_oper_s']/60:.1f} min, SUET = {SAET_s/60:.1f} min")
    ...
    #     (实际出力 = 指令 of ∩ 背压降额 k_p, §2.3.2 统一口径 v3;
    #      强制跳机=背压越限, 出力在部分负荷下被切断 -> ✕ 位于当时实际出力;
    #      主动停机=降出力自然降零 -> ● 位于 0)
    ax = axes[0]
    for rep, style in zip(reps, ["--", "-", ":"]):
        s = [row for row in rep["series"] if row[0] <= rep["T_oper_s"]]
        t = np.array([row[0] for row in s]) / 60.0
        out = np.array([min(row[5], row[6]) for row in s]) * Pg
        lw, col = (2.4, C["warn"]) if rep["tag"] == "critical" else (1.6, C["mut"])
        ax.plot(t, out, style, lw=lw, color=col, label=LBL[rep["tag"]])
        if rep["forced"]:
            ax.plot([t[-1]], [out[-1]], "x", ms=11, mew=2.6, color=C["power"])
        else:
            ax.plot([t[-1]], [out[-1]], "o", ms=9, color=C["ok"])
    ...
    # (b) CWR 轨迹: 强制跳机由背压越限触发(储水消耗->循环流量/冷却能力渐进劣化),
    #     先于储水耗尽 -> ✕ 位于跳机时刻的残余 CWR(约 10% CWR₀), 与曲线末端重合
    ax = axes[1]
    for rep, style in zip(reps, ["--", "-", ":"]):
        s = [row for row in rep["series"] if row[0] <= rep["T_oper_s"]]
        t = np.array([row[0] for row in s]) / 60.0
    ...
    ax.set_xlabel("Time since supply failure (min)")
    ax.set_ylabel("Cooling-water reserve CWR (m³)")
    ...
    out = dict(
        ...
        note="对齐 Yu et al. Nat.Commun. 2024 Fig.3 数值算例（气->水类比）；"
             "市政边界为合成阶跃(B-ST 保守下界)；"
             "v3(2026-09-20)热负荷随实际出力更新 Q=λQ·Pg·min(of,k_p)，"
             "统一 §2.3.2 口径(评审M1-A)。")
