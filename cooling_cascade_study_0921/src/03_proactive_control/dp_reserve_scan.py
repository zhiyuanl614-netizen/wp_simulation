...
v2 (2026-09-15 耦合层重构后): 场景＝S1 单机 bus89 / S2 CO 6 机 / S3 nk worst k=3
（均取自耦合层 6 台总体），horizon 300 min。

(A) DP over R:  3 场景 × 9 R × DP（T_ctrl = 1.5×SAET，α=1.5 与 run_p6/node_sensitivity 一致）
    —— 检验低 R 区 DP 是否优于 SP（更长窗口 → 更多备用爬坡积累，预期 SP 相变点
       按 ≈1/α 下移）。v2 的 k=3 子集 {89,80,10} 不含平衡机，v1 的 57.12 MW
       "网络地板" 不复存在（见 ch5 §5.3 结果三）。
...
严格复用 ProactiveLP（horizon 300 min / dt 5 min / DC-PTDF / 过载权重 5 / PA 因果约束），
除 mode/T_ctrl/reserve_frac 外口径与 ramp_rate_scan.py 完全一致。

回归锚点（容差 0.06）:
  - DP@R=0.01: CO 6 机 = 0 MW/0 MWh/过载 36.76（run_p6 CO 的 DP 行）; 单机 bus89 = 0/0。
...
def main():
    ...
    out = dict(
        ...
        caliber=("ProactiveLP 严格复用（同 ramp_rate_scan）：horizon 300 min, dt 5 min, DC-PTDF, "
        ...
        regression_anchors=anchors, anchors_all_ok=bool(n_ok == len(anchors)),
        notes=[
            ...
            "备用容量型缺额（rf 过低时系统总可用容量 < 负荷 4242 MW）与爬坡型相变（CO/S3 的 R*）为两种可区分机制；v2 总体下不再出现 v1 的网络型地板",
        ],
    )
