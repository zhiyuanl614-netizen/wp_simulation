...
def _forward_water(p, t_fault=0.0, ramp=0.0, out_frac_fn=None,
                   dt=1.0, t_end=12000.0, make_cap_m3s=None):
    """前向积分水力-热力链, 返回 (t_trip, series)。
    out_frac_fn(t): 受影响机组出力指令比例(0..1)时间函数; None=恒定额定。
    t_trip: 机组因高背压/跳泵而退出的时刻 (相对 t_fault)。
    make_cap_m3s: 市政补水封顶 m^3/s (None=不封顶; 评审M2 封顶敏感性实验)。

    v3 (2026-09-20, 评审 M1-A 统一语义): 热负荷随【实际出力】更新
        Q_cond = λ_Q · P_g · min(of, k_p) · 1e3   (k_p = 背压降额反馈),
    与 simulate.py 及论文 §2.3.2 "Heat-load update" 声明定律一致(此前为
    λ_Q·P_g·of 无反馈口径, SUET 偏短 ~4%); 跳机判据同步对齐 simulate.py
    (纯高背压持续计时, 移除"跳泵即跳机"联跳分支); 积分次序同 simulate.py
    (water_derivs 用上一步 Q_cond, 凝汽器后更新)。series 末列追加 k_p。
    """
    ...
    for i in range(n):
        ...
        if (not gen_tripped) and hi_bp_timer >= p.trip_delay_s:
            gen_tripped = True
            t_trip = t
        # ---- 实际出力与热负荷更新: 实际出力 = 指令 ∩ 背压降额 (§2.3.2) ----
        P_aff = 0.0 if gen_tripped else p.Pg_MW * min(of, k_p)
        if gen_tripped:
            Q_cond = max(0.0, Q_cond - dt * p.Q_cond0 / 20.0)   # ~20s残余衰减
        else:
            Q_cond = p.lp_heat_frac * P_aff * 1000.0
        series.append((t, Ht, Hp, m_cw, p_b, of, k_p))
...
# v2 (2026-09-15): 受影响机组集合由耦合层 coupling_map.json 单一真源读取
...
def coupled_buses():
    ...
    cm = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..",
                      "results", "muni", "coupling_map.json")
    try:
        return tuple(r["bus"] for r in json.load(open(cm))["map"])
    except Exception:
        return COUPLED_BUSES


if __name__ == "__main__":
    for b in coupled_buses():
