...
作为扫描变量**, 其余口径与 run_p6 / nk_scan 完全一致:
  - horizon 300 min (由 200 扩, 覆盖 6 台中最大 SAET 192.9 min 的软着陆窗口; v4 统一语义),
...
def main():
    ...
    # v3 (2026-09-20, 评审M1-A 统一语义): ANCHOR_MODE=report 供语义变更后首轮收值
    # (收值后更新 ANCHORS 表, 须以默认 strict 模式复跑并通过方为注册结果)
    if os.environ.get("ANCHOR_MODE", "strict") == "report":
        print(f"[ANCHOR_MODE=report] 锚点 {len(anchors) - n_ok}/{len(anchors)} 项不一致"
              f" —— 语义统一后待更新锚点表, 本轮结果不作注册")
    else:
