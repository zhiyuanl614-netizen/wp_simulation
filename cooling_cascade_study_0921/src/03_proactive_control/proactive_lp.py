...
class ProactiveLP:
    def __init__(self, affected_buses=(89, 80, 10), horizon_min=40.0, dt_min=2.0,
        ...
        self.pa_traj = pa_traj   # PA 可用出力轨迹: "step"=跳机时刻阶跃(注册口径);
                                 # "derate"=凝汽器降额轨迹 Pg·k_p(t) (评审M5 复算实验)
    ...
    def _aff_avail_traj(self, mode, T_ctrl=None):
        ...
        for k, gi in enumerate(self.aff_idx):
            ...
            else:  # PA 被动: step=水力跳机时刻阶跃降零; derate=降额轨迹 Pg·k_p(t) (评审M5)
                Pg, Pmax = load_unit_from_gen(bus=b)
                p = Params(Pg_MW=Pg, Pmax_MW=Pmax)
                t_trip, series = wi._forward_water(
                    p, out_frac_fn=None,
                    t_end=max(12000.0, (self.T * self.dt + 30.0) * 60.0))
                t_trip_min = (t_trip or 1e9) / 60.0 + off
                if self.pa_traj == "derate":
                    ts = np.array([r[0] for r in series]) / 60.0    # min, 自危机起点
                    kp = np.array([r[6] for r in series])
                    trip_cut = (t_trip / 60.0) if t_trip is not None else np.inf
                    traj = []
                    for t in range(self.T):
                        tt = t * self.dt - off
                        if tt < 0.0:
                            traj.append(Pg_i)                       # 市政失压前满出力
                        elif tt >= trip_cut:
                            traj.append(0.0)                        # 跳机后
                        else:
                            traj.append(Pg_i * float(np.interp(tt, ts, kp)))
                    avail[k] = np.array(traj)
                else:
    ...
    # ---- 组装并求解 LP ----
    def solve(self, mode="SP", T_ctrl=None, first_trip_override_min=None, w_ovl=5.0):
        """mode: 'PA'/'SP'/'DP'. T_ctrl: SP/DP 各受影响机组控制时间(min)列表。
        w_ovl: 过载松弛惩罚权重(仅 DC 软约束时使用), 默认 5.0。
        ...
        if use_ovl:
            for t in range(T):
                for l in range(nl):
                    c[oidx(l, t)] = w_ovl           # 过载惩罚权重(> 缺额, 尽量不过载)
