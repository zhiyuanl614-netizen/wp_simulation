...
单步执行顺序(自下而上采集, 自上而下下发):
  Step 1  物理层水力-热力推进(复用 p1_smib/submodels.py)
  Step 2  PLC 采样传感器(市政/水箱/池/流量/背压)
  Step 3  SCADA 汇集归档 + 故障检测(策略A/B/C/D) + 告警
  Step 4  调度中心决策 -> 是否发出/送达跨域预警(含通信时延)
  Step 5  预警送达电网 -> 电网触发主动处置(备用预起机+runback+预防切负荷)
  Step 6  电网侧准稳态频率(用 P4/P5 同款分级备用模型)
  Step 7  快照归档 + 时间推进

关键: 预警送达时刻由 ICS 内生(检测策略 + 通信时延)决定,
      再换算为电网侧 proactive 控制的等效 t_detect。
"""
import os, sys
import numpy as np
...
class ICSCoolingGridSim:
    """
    单机(SMIB 型)冷却水—ICS—电网耦合仿真。
    电网侧采用 P4/P5 同款"受影响机组 + 系统其余部分 + 分级备用"准稳态频率模型,
    但主动处置的触发时刻由 ICS 预警决定(而非手填 t_detect)。
    """

    def __init__(self, bus=89, warning_enabled=True, t_fault=60.0, ramp=0.0,
                 sample_dt=1.0,
                 reserve_boost=0.0, preemptive_shed=0.0,
                 runback_rate_frac=0.0015, db_reset=True):
        ...
        self.reserve_boost = reserve_boost
        self.preemptive_shed = preemptive_shed
        ...
        # 电网侧(SMIB: 受影响机组 + 系统其余部分)
        P_rest_rated = Pmax * 6.0
        self.Sbase = Pmax + P_rest_rated
        self.P_aff0 = Pg
        self.P_rest_rated = P_rest_rated
        self.P_rest0 = P_rest_rated * (Pg / Pmax)
        self.P_load0 = self.P_aff0 + self.P_rest0
        self.f0 = 50.0
        self.R_sys = 0.05
        self.D_load = 1.0
        self.reserve_frac = 0.10
        self.tertiary_cap = 300.0                 # 单机场景三级备用 MW
        self.tertiary_rate = 300.0 / 900.0        # MW/s
    ...
    def step(self, t, dt):
        ...
        # ---- Step 5: 电网主动处置(预警送达后) ----
        # runback: 速率受限(不快于备用起机速率)
        if warned and not self.gen_tripped:
            aff_total = self.P_aff0
            rate_cap = self.tertiary_rate / max(aff_total, 1.0)
        ...
        lost = self.P_aff0 - P_aff
        # 三级备用起机: 预警->提前起机; 否则跳机后起机
        if warned:
            elapsed = max(0.0, t - self.warn_arrival_t)
        else:
            elapsed = 0.0 if self.t_gen_trip is None else max(0.0, t - self.t_gen_trip)
        tert_online = min(self.tertiary_cap, self.tertiary_rate * elapsed)
        # 预防性切负荷(预警后)
        shed_MW = 0.0
        if warned and self.preemptive_shed > 0:
            shed_cap = self.preemptive_shed * self.P_load0
            shed_ramp = shed_cap / 120.0
            shed_MW = min(shed_cap, shed_ramp * max(0.0, t - self.warn_arrival_t))
        spin = (self.reserve_frac + (self.reserve_boost if warned else 0.0)) * self.P_rest_rated
        reserve_avail = spin + tert_online + shed_MW
        pickup = min(lost, reserve_avail)
        unmet = max(0.0, lost - pickup)
        D_term = self.P_load0 * self.D_load
        ...
        self.db.write_snapshot(t, phys["muni_head"], self.H_tank, self.H_pool,
                               ...
                               1 if self.gen_tripped else 0, f,
        ...
        self.rows.append(dict(t=t, muni=phys["muni_head"], H_tank=self.H_tank,
                              ...
                              out_frac=out_frac, f=f, warned=int(warned),
                              lost=lost, pickup=pickup))

        self.H_tank = max(p.H_tank_min, self.H_tank + dt * dHt)
        self.H_pool = max(0.0, self.H_pool + dt * dHp)

    def run(self, t_end=2600.0, dt=4.0):
        n = int(t_end / dt) + 1
        for i in range(n):
            self.step(i * dt, dt)
        self.db.commit()
    ...
    def summary(self):
        f = np.array([r["f"] for r in self.rows])
        warned_rows = [r for r in self.rows if r["warned"]]
        return dict(
            ...
            ics_latency=self.dispatch.total_latency(),
            t_gen_trip=self.t_gen_trip,
            lead_min=(None if (self.warn_arrival_t is None or self.t_gen_trip is None)
                      else round((self.t_gen_trip - self.warn_arrival_t) / 60.0, 2)),
            f_nadir=round(float(f.min()), 3),
        )
