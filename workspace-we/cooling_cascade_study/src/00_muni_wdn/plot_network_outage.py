...
def main():
    ...
    ax1.text(tf + 0.5, ax1.get_ylim()[1] * 0.88, f"Unique source\noutage t={tf:.0f} h",
             color=COLORS["power"], fontsize=8.5)
    ax1.set_xlabel("Time (h)")
    ax1.set_ylabel("Node pressure head (m)")
    ax1.set_title("(a) Network-wide pressure statistics vs time", fontsize=11)
    ...
    ax2.set_title(f"(b) Failed-node fraction (head < {thr:.0f} m) vs time", fontsize=11)
    ax2.set_ylim(0, 100)
    ...
    for col, h in enumerate(o["snap_hours"]):
        ...
        tag = "pre-outage" if h < tf else f"+{h-tf:.0f} h"
        ax.set_title(f"(c{col+1}) t={h} h ({tag})\nfailed {frac:.0f}%", fontsize=9.5)
    ...
    fig.suptitle("Spatio-temporal pressure collapse after unique-source (R1) outage: "
                 "tanks sustain supply, then deplete \u2192 failure spreads outward from source",
