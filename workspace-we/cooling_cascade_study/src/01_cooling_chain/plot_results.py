...
def plot_one(path, t_fault=60.0, also_fig=False):
    ...
    fig.suptitle("Cooling-water fault mechanism chain: "
                 "water cutoff \u2192 pump trip \u2192 back-pressure rise \u2192 unit trip \u2192 power loss",
                 fontsize=12.5)
    ...
    a.set_ylabel("Water level (m)"); a.set_title("(a) Water-source levels"); a.legend(); mark(a)

    a = ax[0, 1]
    a.plot(t, d["m_cw"], color=C["w"])
    a.set_ylabel("$m_{cw}$ (m$^3$/s)"); a.set_title("(b) Circulating-water flow"); mark(a)
    ...
    a.set_ylabel("Back-pressure (kPa)"); a.set_title("(c) Condenser back-pressure"); a.legend(); mark(a)

    a = ax[1, 1]
    a.plot(t, d["k_p"], color=C["k"])
    a.set_ylabel("$k_p$"); a.set_title("(d) Turbine power derating factor"); a.set_ylim(0, 1.1); mark(a)

    a = ax[2, 0]
    a.plot(t, d["Paff_MW"], color=C["b"])
    a.set_ylabel("Output (MW)"); a.set_xlabel("Time (min)")
    a.set_title("(e) Affected-unit output"); mark(a)

    a = ax[2, 1]
    a.plot(t, d["lost_MW"], color=C["f"])
    a.set_ylabel("Power loss (MW)"); a.set_xlabel("Time (min)")
    a.set_title("(f) Unit power loss (impact metric)"); mark(a)
