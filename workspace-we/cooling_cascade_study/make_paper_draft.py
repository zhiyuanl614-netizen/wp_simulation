...
<h3>3.1　测试系统与耦合配对</h3>
<p class="summary">介绍 IEEE-118（电）× D-town（水）双网案例与取水节点↔电厂母线的耦合对；如实交代配对为功能类型指定（依据见文末诚实性说明），并以 §5 敏感性回应配对任意性。</p>
...
<p class="summary">展示同一水源失效经不同分区缓冲导致各取水节点错峰失压，以及唯一水源停供后全网压力自水源向外的时空崩溃，作为时间差的市政侧来源与边界真实性证据。</p>
{figure("Fig5_muni_staggered_depressurization.png", "图 5　市政总源失效 → 各取水节点错峰失压（§4.1）")}
...
<p class="summary">用时序结果证明冷却水储水缓冲把市政断水延迟约 SAET≈92 min 才传导到机组跳机，直观展示被忽略的冷却水过程如何"制造"可用时间差。</p>
...
<h3>6.4　局限</h3>
<p class="summary">如实列出局限：无现场数据、解析水力、DP 单次近似、耦合配对为指定。</p>
